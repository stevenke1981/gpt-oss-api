...
$Presets = @(
    ...
    [pscustomobject]@{ Name="全GPU 6k";  Desc="RTX 3060 推薦，Flash Attn 啟用，速度最快";         GPU=-1; CTX=6144; Predict=-1; Parallel=1; Threads=8 }
    [pscustomobject]@{ Name="全GPU 8k";  Desc="最大實用 context（VRAM 約剩 230 MiB）";            GPU=-1; CTX=8192; Predict=-1; Parallel=1; Threads=8 }
    [pscustomobject]@{ Name="全GPU 多人"; Desc="4 槽平行，多用戶使用";                             GPU=-1; CTX=4096; Predict=-1; Parallel=4; Threads=8 }
)
